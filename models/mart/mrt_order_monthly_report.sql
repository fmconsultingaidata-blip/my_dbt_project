...
total_monthly_user_from_jawa_timur AS (
    SELECT 
        DATE_TRUNC(orders.order_created_at, MONTH) AS order_month,
...
SELECT 
    mr.order_month,
    COALESCE(mr.total_monthly_users, 0) AS total_users_monthly,
    COALESCE(jt.total_monthly_users_from_jawa_timur, 0) AS total_monthly_user_java_timur,
    COALESCE(mr.total_monthly_orders, 0) AS total_monthly_order_count
